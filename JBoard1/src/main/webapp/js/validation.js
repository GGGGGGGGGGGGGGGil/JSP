...
let regName  = /^[가-힣]{2,4}$/;
let regNick  = /^[가-힣a-zA-Z0-9]+$/; //+ = 순서없이 한번씩 나옴
...
$(function(){
	...
	$('#btnUidCheck').click(function(){
		...
		let uid = $('input[name=uid]').val(); //사용자 입력한 값 가져오기
		
		//중복체크 연속으로 눌러도 ajax 실행 안되게 하기
		if(isUidOk){
			return; //클릭하면 실행x
		}
		
		
		//서버로 전송되기 전에 잘못된 아이디 걸러내기
		if(!uid.match(regUid)){ //text = x, match 함수 써야됨
			isUidOk = false;
		...
		setTimeout(()=>{
			...
			$.ajax({
				...
				method: 'get', //전송데이터 한개, 간편하게 get으로
				data: jsonData,
				dataType: 'json',
	...
	$('#btnNickCheck').click(function(){
		...
		//서버로 전송되기 전에 잘못된 아이디 걸러내기
		if(!nick.match(regNick)){ //text = x, match 함수 써야됨
			isNickOk = false;
			$('.resultNick').css('color', 'red').text('별명 유효하지 않습니다.');
			return;
		}
